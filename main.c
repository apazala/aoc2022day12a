...
#define valid(u, v) ((v).i >= 0 && (v).i < nrows && (v).j >= 0 && (v).j < ncols && stepsNeeded[(v).i][(v).j] < 0 && mountain[(v).i][(v).j] - mountain[(u).i][(u).j] <= 1)
...
int main()
{
    ...
    stepsNeeded[p.i][p.j] = 0;
    pos r;
    while (p.i != target.i || p.j != target.j)
    {
        // up?
        r.i = p.i - 1;
        r.j = p.j;
        ...
        p = pop(queue); // Unchecked, but could be empty if target is unreachable
    }

    printf("%d\n", stepsNeeded[target.i][target.j]);
}
